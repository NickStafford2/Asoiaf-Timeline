...
export class MomentService implements timelineItemStore {
  ...
  private _moment$: BehaviorSubject<Array<NSMoment>> = new BehaviorSubject<Array<NSMoment>>([]);
  ...
  public loadAllMoments() {
    this.momentService.getMoments().subscribe((response: Array<NSMoment>) => {
  ...
  public getMoments(): Array<NSMoment> { 
    const copy: Array<NSMoment> = [];
